...
public class AiInvocationAuditService {
    ...
    public boolean reserve(AiInvocation invocation, BigDecimal dailyBudget) {
        repository.lockTenantBudget(invocation.tenantId());
        Instant from = LocalDate.ofInstant(invocation.startedAt(), ZoneOffset.UTC)
                .atStartOfDay(ZoneOffset.UTC).toInstant();
        Instant to = from.plusSeconds(86_400);
        BigDecimal exposure = repository.sumDailyExposure(invocation.tenantId(), from, to);
    ...
    public AiInvocation get(String tenantId, UUID id) {
        Optional<AiInvocation> invocation = repository.findByTenantAndId(tenantId, id);
