...
import dev.ainer.module.notification.notification.infrastructure.SmtpMailChannelSender;
...
public class NotificationModuleConfiguration {
    ...
    @ConditionalOnProperty(prefix = "ainer.notification.email", name = "enabled", havingValue = "true")
    SmtpMailChannelSender emailSender(
            ObjectProvider<JavaMailSender> mailSender,
            NotificationEmailProperties properties) {
        JavaMailSender sender = mailSender.getIfAvailable();
        if (sender == null) {
            throw new IllegalStateException(
                    "ainer.notification.email.enabled requires a JavaMailSender (configure spring.mail.host)");
        }
        return new SmtpMailChannelSender(sender, properties);
    }
}
