...
> 文档类型：长期维护规范 · 状态：生效 · 最近核对：2026-08-28 · 适用版本：`1.4.x+`
...
5. 本地 non-SNAPSHOT Maven 3.9+/Maven 4 Golden Consumer 与 Initializer 五项目门禁通过；
...
普通/PostgreSQL/CRUD、v2 安全纵向切片四个全新项目，以及从安全项目演化的 existing-project
增量项目，必须执行各自生成的 Maven 3.9.16 Wrapper。
发布后的远端门禁使用：
...
`github-packages`。2026-08-28 `v1.4.1` 核对：28 个 Maven 包均为 public（`0.1.0` /
`0.2.0` / `1.0.0` / `1.2.0` / `1.3.0` / `1.4.0` / `1.4.1`）。新包在首次 deploy 后仍应核对其 `visibility`，不得
...
8. 确认远端制品完整可见后，从两个空仓运行 Maven 3/4 consumer，并从远端获取 CLI 生成带 Wrapper
   的项目，再以生成 Wrapper 运行 Initializer 五项目门禁；
