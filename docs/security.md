...
通知读写/提交、字典读写、任务读写/提交、知识读写/发布、`ai.invoke` 与 `ai.agents.manage`
接线 `@AinerAuthorize`，并注册 Workspace 路径 `AuthorizationTargetResolver`。**不是** 1.x
资源级授权合同。网关仅在请求带 `actingAgentId` 时调用 `ActingGrant.check`（缺 `workspaceId`
返回 422，拒绝不泄露 reason）。组织 Controller、把 permission 改成类型化 resourceType、
