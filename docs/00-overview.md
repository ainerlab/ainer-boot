...
> 文档类型：统一入口 · 状态：生效 · 最近核对：2026-08-28 · 适用版本：`1.4.1` 当前稳定（`v1.1.0` withdrawn；Packages `0.1.0`/`0.2.0`/`1.0.0`/`1.2.0`/`1.3.0`/`1.4.0`/`1.4.1` 已 public）
...
| [`decisions/0057-identity-lifecycle-write-path-and-security-epoch.md`](decisions/0057-identity-lifecycle-write-path-and-security-epoch.md) | ADR-0057：账号/服务主体生命周期写路径与 `security_epoch` 递增语义（撤销即时性限于在线校验路径） |
