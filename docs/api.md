...
通用错误：400 `AINER.COMMON.INVALID_REQUEST`、401 `AINER.COMMON.UNAUTHENTICATED`、403 `AINER.COMMON.FORBIDDEN`、404 `AINER.COMMON.NOT_FOUND`、405 `AINER.COMMON.METHOD_NOT_ALLOWED`、406 `AINER.COMMON.NOT_ACCEPTABLE`、409 `AINER.COMMON.CONFLICT`、415 `AINER.COMMON.UNSUPPORTED_MEDIA_TYPE`、422 `AINER.COMMON.BUSINESS_RULE_VIOLATION`、429 `AINER.COMMON.RATE_LIMITED`、500 `AINER.COMMON.INTERNAL_ERROR`、503 `AINER.COMMON.SERVICE_UNAVAILABLE`。模块可以提供更具体的稳定错误码。

Spring MVC 自身判定的异常同样保留真实状态码：405（带 `Allow` 响应头）、406、415（带 `Accept` 响应头）与非法请求体 400 都返回上述信封，不会退化成 500。客户端 `Accept` 不接受 JSON 时（例如 406 场景），错误信封仍以 `application/json` 返回，保证真实状态码与稳定错误码同时可见；未单独列举的 4xx/5xx 保留真实状态码，`code` 回落到最接近的通用错误码。
