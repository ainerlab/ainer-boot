...
- **H5'**：不改默认值。`operations.md` / `configuration.md` / `security.md` 把在线校验与
  step-up 标为对外生产签发前必选项。M1/M3/L 系/M5 此前已关闭；M5' 参考装配已接线
  （不是 1.x 资源级授权合同）。仍开放 M2 延迟自提权（见评审快照后续关闭表）。
...
- **遗留 follow-up**（当时记录）：workspace ALLOWED 审计顺序（M3）、自提权创建后防护（M2）、
  ...
  **后续**：M1/M3/L 系/M5 已于 2026-08-20 关闭，H5' 文档于 2026-08-25 关闭；M5' 参考接线于
  2026-08-25 关闭切片。仍开放 M2 延迟自提权——详见评审快照后续关闭表。
