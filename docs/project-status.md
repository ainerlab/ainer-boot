...
Foundation 已进入 M4.8A 控制面代码收口：M4.7 tenant 成员管理与首租户严格 bootstrap 已落地，平台
...
生产限速/告警和 0-skipped 发布门禁尚未完成。当前工程是可编译、可运行、可用真实
...
- ADR-0001 至 ADR-0011、ADR-0015 至 ADR-0020 已接受，ADR-0012 至 ADR-0014 及 ADR-0021 处于 Proposed；
  架构、HTTP API、安全、数据、测试、运行与发布基础文档已建立。

## 3. 最近验证证据

2026-07-26 在 macOS Colima、Testcontainers 2.0.5 与 `postgres:18.3-alpine` 环境执行完整
`mvn test`：14 个 Reactor 模块成功，51 个测试套件、221 个测试全部实际执行通过，
