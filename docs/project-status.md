...
- **版本选择**：仅修复商业文档/发布治理与测试夹具确定性，不改变运行时 Java/HTTP/错误码/JWT/
  配置/数据库/事件/Initializer 合同，也不改变 28 modules / 132 主制品清单，按 ADR-0045 选择
  **PATCH `1.4.1`**。
- **本地验证**：JDK 25 + Maven 4.0.0-rc-6 + Colima/PostgreSQL 18.3 下，完整
  `./mvnw clean verify` 为 549 tests / 0 failure / 0 error / 0 skipped；商业文档正向门禁对
  `v1.4.1` 通过，伪造 `v1.5.0` 目标版本会失败关闭。远端 CI、目标版本制品、tag、Release 与
  132/132 远端读回仍待本次发布流程完成，不能提前记为发布证据。
...
- **文档**：`LICENSE`、`NOTICE`、根 POM `<licenses>`；README / handoff / releasing /
  商业文档的当前许可表述已对齐。2026-08-04「暂不开源」历史记录保留。
...
- **P3**：`KnowledgeController` 消费 `@AinerAuthorize`（`knowledge.read` /
  ...
  模块 HTTP 切片未装配拦截器。请求体/查询 `workspaceId` 不作为授权目标。**不是** 按对象/
  修订 ID 的对象级合同。

2026-08-25 任务端点授权粗门禁
- **P3**：`TaskController` 消费 `@AinerAuthorize`（`task.read` / `task.manage` /
  `task.submit`）。LivePath：无 Binding→403，有 `task.submit` Binding→200。模块 HTTP
  切片未装配拦截器。**不是** 按作业 ID 的对象级合同。
...
- **P3**：`NotificationManagementController` 消费 `@AinerAuthorize`（`notification.read` /
  ...
  `notification.submit` Binding→200。模块 HTTP 切片未装配拦截器。**不是** 按模板/记录
  ID 的对象级合同。
...
- **剩余动作**：
  1. ~~解除配额~~ → **ADR-0048（选项 C）已执行**：退役 rc 链三个版本共 69 个条目
     （25 包 × 3 版本；`ainer-module-task` 从未发布故不存在），每包现恰好含
     `0.1.0/0.2.0/1.0.0`，包体降至约 188MB；
  2. **402 根因最终定性（第七次尝试后确认）**：免费额度按"存储 GB·小时/月"计——
     本月账单显示 Actions storage 290.9 GBh + Packages storage 81.1 GBh = **372 GBh，
     恰好等于 500MB×744h 的月度配额**，全部被折扣抵扣（净额 $0）。此前 ~1.5GB 的
     缓存/制品持有数周已把当月配额耗尽；删除只降低未来累计速率，不能追回当月已
     消耗的 GB·小时。因此写入（deploy 402）与内容读取锁定在计费月重置前持续，
     与当前瞬时占用无关；
  3. **两条解锁路径**：① 负责人在 Billing → Spending limits 把上限调为 >$0——超额
     GB·小时按约 $0.000336/GBh 计费（金额为分级美分），立即解锁；② 等待 9 月 1 日
     计费月重置后以精简基线（~188MB）自然回到额度内；
  4. 解锁后完成发布：确认缓存为 0 → `gh run rerun 32725167463`（tag `be9cb8f` 已含
     全部修复与 ADR-0048）→ 双消费者 `1.0.0 → 1.1.0` 升级矩阵。
...
- **执行**：三路并行批量翻译（仅动注释不改代码行，逐模块编译验证）——P3 四件套、
  workspace+authorization（含评审点名的缺失类级 javadoc 补写）、框架层 core/spring/
  security/starter×4 + identity + ai-gateway。私有方法单句英文行注释允许保留不强制。
- 测试注释（src/test）本轮不动，后续按需。
...
- 受控逃逸舱仅有文档决策，工程验收未完成：模块 SPI 端口的补齐、消费者侧 `VENDORED.yaml`
