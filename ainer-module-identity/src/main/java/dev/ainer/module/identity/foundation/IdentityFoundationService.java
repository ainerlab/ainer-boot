 ...
 *
 * <p>Exercises {@link HumanAccount} + {@link LoginIdentity} end-to-end without a database, via the
 * {@link HumanAccountRepository} / {@link LoginIdentityRepository} ports. This is the working registration
 * core that the destructive cutover eventually persists and wires into the Authorization Server; it is
 * deliberately decoupled from the legacy tenant-bound services and does not touch them.
 *
 * <p>Collision handling throws {@link IllegalStateException} in this skeleton; the production wiring replaces
 * it with {@code BusinessException(IdentityErrorCode)} and a dedicated conflict error. Identifier equality
 * never auto-merges accounts — a duplicate {@code (type, providerAuthority, normalizedIdentifier)} is a hard
 * conflict.
 */
public class IdentityFoundationService {
    ...
    public LoginIdentity linkLoginIdentity(
        ...
        HumanAccount account = accountRepository.findByAccountId(accountId)
                .orElseThrow(() -> new IllegalStateException("HumanAccount not found: " + accountId));
        if (!account.status().canAuthenticate()) {
            throw new IllegalStateException("HumanAccount is not active: " + accountId);
        }
        requireNoCollision(type, providerAuthority, normalizedIdentifier);
    ...
    private void requireNoCollision(
        ...
        if (loginIdentityRepository.findByTypeAndIdentifier(type, providerAuthority, normalizedIdentifier)
            ...
            throw new IllegalStateException(
                    "LoginIdentity already exists: " + type + " / " + providerAuthority
