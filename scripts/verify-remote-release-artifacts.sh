...
[[ "$project_count" == "28" ]] \
  || fail "release artifact manifest must contain 28 projects, found $project_count"
[[ "$expected_primary_count" == "132" ]] \
  || fail "release artifact manifest must describe 132 primary artifacts, found $expected_primary_count"
...
- Maven artifacts: 132 primary files, each read back from GitHub Packages with a valid detached OpenPGP signature
