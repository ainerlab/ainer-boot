...
public final class AinerPasskeyEnrollmentGrantService {
    ...
    public AccountEnrollmentGrant grantForAccount(
            String grantedBy, UUID accountId, String incidentReference) {
        requireSafe(grantedBy, "grantedBy");
        requireSafe(incidentReference, "incidentReference");
        ...
        return transactionTemplate.execute(status -> {
            ...
            int upserted = jdbcTemplate.update(
                    """
                    INSERT INTO ainer_passkey_enrollment_grant(
                        subject_id, account_id, tenant_id, granted_by, incident_reference,
                        status, granted_at, consumed_at
            ...
            securityAuditForAccount(accountId, grantedBy, incidentReference, "GRANTED", now);
            return new AccountEnrollmentGrant(
                    accountId, grantedBy, incidentReference, ACTIVE, now, null);
        });
    }
    ...
    public AccountEnrollmentGrant revokeForAccount(String revokedBy, UUID accountId) {
        requireSafe(revokedBy, "revokedBy");
        Assert.notNull(accountId, "accountId cannot be null");
        return transactionTemplate.execute(status -> {
            Instant now = clock.instant();
            ...
            securityAuditForAccount(accountId, revokedBy, "revoked", "REVOKED", now);
            return new AccountEnrollmentGrant(
                    accountId, revokedBy, "revoked", REVOKED, now, null);
        });
    }
    ...
    private void securityAuditForAccount(
        ...
        jdbcTemplate.update(
                ...
                INSERT INTO ainer_passkey_security_operation_audit(
                    ...
                    actor_type, actor_id, incident_reference, request_id, occurred_at
                ) VALUES (?, ?, NULL, NULL, ?, 'ENROLLMENT_GRANT', ?, 'SERVICE', ?, ?, ?, ?)
                """,
                UUID.randomUUID(), UUID.randomUUID(), accountId, phase, actorServiceId,
