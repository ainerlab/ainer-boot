...
final class AinerClientControlOperatorBootstrapRunner implements ApplicationRunner {
    ...
    AinerClientControlOperatorBootstrapRunner(
    ...
    public void run(ApplicationArguments args) {
        ...
        if (clientId == null || !IDENTIFIER.matcher(clientId).matches()) {
            throw new IllegalStateException(
                    "Ainer client-control operator bootstrap client id is invalid");
        }
        ...
        if (secret == null || secret.length() < 24 || secret.length() > 128) {
            throw new IllegalStateException(
                    "Ainer client-control operator bootstrap secret must contain 24 to 128 characters");
        }
        RegisteredClient client = RegisteredClient.withId(UUID.randomUUID().toString())
                .clientId(clientId)
                .clientSecret(passwordEncoder.encode(secret))
                .clientName("Ainer client-control operator")
                .clientAuthenticationMethod(ClientAuthenticationMethod.CLIENT_SECRET_BASIC)
                .authorizationGrantType(AuthorizationGrantType.CLIENT_CREDENTIALS)
                .scope(AinerAuthorizationServerConfiguration.CLIENT_CONTROL_MANAGE_SCOPE)
