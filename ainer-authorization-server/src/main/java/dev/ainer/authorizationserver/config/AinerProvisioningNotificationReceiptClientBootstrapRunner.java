...
final class AinerProvisioningNotificationReceiptClientBootstrapRunner
    ...
    public void run(ApplicationArguments args) {
        ...
        if (clientId == null || !IDENTIFIER.matcher(clientId).matches()) {
            throw new IllegalStateException(
                    "Ainer provisioning notification receipt bootstrap client id "
                            + "is invalid");
        }
        ...
        if (secret == null || secret.length() < 24 || secret.length() > 128) {
            throw new IllegalStateException(
                    "Ainer provisioning notification receipt bootstrap secret "
        ...
        RegisteredClient client = RegisteredClient.withId(
                ...
                .clientName("Ainer provisioning notification receipt gateway")
    ...
    private void requireExactExistingClient(RegisteredClient client) {
        ...
        if (!client.getScopes().equals(Set.of(
            ...
            throw new IllegalStateException(
                    "Existing Ainer provisioning notification receipt client "
