...
public enum AuthorizationErrorCode implements ErrorCode {
    ...
    SET_BINDING_NOT_FOUND("AINER.AUTHORIZATION.SET_BINDING_NOT_FOUND", "集合绑定不存在", 404),
    SET_BINDING_ALREADY_REVOKED("AINER.AUTHORIZATION.SET_BINDING_ALREADY_REVOKED", "集合绑定已撤销", 409),
    ACTING_GRANT_NOT_FOUND("AINER.AUTHORIZATION.ACTING_GRANT_NOT_FOUND", "委托授权不存在", 404),
    ACTING_GRANT_ALREADY_REVOKED("AINER.AUTHORIZATION.ACTING_GRANT_ALREADY_REVOKED", "委托授权已撤销", 409),
    UNKNOWN_SUBJECT_SET("AINER.AUTHORIZATION.UNKNOWN_SUBJECT_SET", "集合类型没有已注册的成员解析器", 422),
    SUBJECT_SET_PERMISSION_FORBIDDEN("AINER.AUTHORIZATION.SUBJECT_SET_PERMISSION_FORBIDDEN",
            "高风险或系统专用权限不得通过集合绑定授予", 422),
    SUBJECT_SET_SCOPE_MISMATCH("AINER.AUTHORIZATION.SUBJECT_SET_SCOPE_MISMATCH",
            "集合归属与授权范围不属于同一 Workspace", 422);
